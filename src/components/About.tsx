...
export default function About() {
  ...
  const { bio, strengths, qualities } = data;
    ...
    <section id="apropos" ref={sectionRef} className="py-24 px-6 bg-surface relative">
      <div className="max-w-6xl mx-auto">
        <div className={`text-center mb-16 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <p className="text-text-secondary max-w-2xl mx-auto">Découvrez qui je suis, ce qui me motive et comment je peux vous aider.</p>
        </div>
        ...
        <div className={`glass-card p-8 mb-12 transition-all duration-700 delay-200 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <p className="text-text-primary text-lg leading-relaxed mb-4">{bio.short}</p>
          <p className="text-text-secondary leading-relaxed mb-4">{bio.full}</p>
          <p className="text-accent font-medium italic">{bio.motivation}</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className={`transition-all duration-700 delay-400 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"}`}>
            <h3 className="text-xl font-bold font-[Poppins] mb-4 text-text-primary">Compétences Techniques</h3>
            <div className="glass-card p-6 mb-4">
              <h4 className="text-success font-semibold text-sm uppercase tracking-wider mb-3">Points Forts</h4>
              <div className="flex flex-wrap gap-2">
                {strengths.technical.strong.map((s) => (
                  <span key={s} className="px-3 py-1.5 rounded-lg bg-success/10 text-success text-sm border border-success/20">{s}</span>
                ))}
              </div>
            </div>
            <div className="glass-card p-6">
              <h4 className="text-amber-400 font-semibold text-sm uppercase tracking-wider mb-3">Points Faibles</h4>
              <div className="flex flex-wrap gap-2">
                {strengths.technical.toImprove.map((s) => (
          ...
          <div className={`transition-all duration-700 delay-500 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}>
            <h3 className="text-xl font-bold font-[Poppins] mb-4 text-text-primary">Soft Skills</h3>
            <div className="glass-card p-6 mb-4">
              <h4 className="text-success font-semibold text-sm uppercase tracking-wider mb-3">Points Forts</h4>
              <div className="flex flex-wrap gap-2">
                {strengths.soft.strong.map((s) => (
                  <span key={s} className="px-3 py-1.5 rounded-lg bg-success/10 text-success text-sm border border-success/20">{s}</span>
                ))}
              </div>
            </div>
            <div className="glass-card p-6">
              <h4 className="text-amber-400 font-semibold text-sm uppercase tracking-wider mb-3">Points Faibles</h4>
              <div className="flex flex-wrap gap-2">
                {strengths.soft.toImprove.map((s) => (
                  <span key={s} className="px-3 py-1.5 rounded-lg bg-amber-400/10 text-amber-400 text-sm border border-amber-400/20">{s}</span>
                ))}
              </div>
            </div>
